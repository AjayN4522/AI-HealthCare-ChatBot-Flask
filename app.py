...
# Function to read CSV file and return a dictionary of usernames and hashed passwords
...
def check_credentials(username, password):
    names=read_users_from_csv().keys()
    uname=list(names)
    passwords=read_users_from_csv().values()
    pwd=list(passwords)
    print(uname,username,pwd,password)
    if username in uname and password in pwd:
        return True
    return False
...
def register():
    if request.method == 'POST':
        ...
        df_users = pd.read_csv('users.csv', header=None, names=['username', 'password'])
...
def get_bot_response():
    ...
    if session['step'] == "BYE":
        ...
        if s.lower() == "yes":
            ...
            return "THANKS Mr/Ms " + name + " for using me for more information please contact <b> +9080845065</b>"
